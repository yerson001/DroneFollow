...
//Constructor
Particle::Particle(Mat &_imagePatch, Mat &_inputFrame, int _nParticles, Size _patchSize, Point &_ROI)
: nParticles(_nParticles), patchSize(_patchSize), ROI(_ROI)
{
    imagePatch = _imagePatch.clone();
    inputFrame = _inputFrame.clone();

//    imshow("refFrane"+std::to_string(_nParticles),imagePatch);
//    imshow("videoframe"+std::to_string(_nParticles),inputFrame);
